...
class Project(models.Model):
    ...
    def total_seconds(self):
        return sum(
            s.useful_seconds for s in self.sessions.filter(ended_at__isnull=False)
        )

    @property
    def total_sessions(self):
        return self.sessions.filter(ended_at__isnull=False).count()
...
class Session(models.Model):
    ...
    def coding_seconds(self):
        ...
        for e in events:
            if e.event_type == "server_start":
                total += int((e.timestamp - last_time).total_seconds())
                in_test = True
                last_time = e.timestamp
            elif e.event_type == "server_stop":
                in_test = False
                last_time = e.timestamp
    ...
    def testing_seconds(self):
        ...
        for e in events:
            if e.event_type == "server_start":
                last_time = e.timestamp
            elif e.event_type == "server_stop" and last_time:
                total += int((e.timestamp - last_time).total_seconds())
                last_time = None
        return total

    @property
    def useful_seconds(self):
        return self.coding_seconds + self.testing_seconds
