...
class TestNewProject(TestCase):
    ...
    def test_nickname_duplicado_retorna_erro(self):
        ...
        with patch(
            "sys.argv", ["new-project", "--name", "Outro", "--nickname", "meuapp"]
        ):
            with captured_output():
    ...
    def test_nickname_com_espaco_retorna_erro(self):
        with patch(
            "sys.argv", ["new-project", "--name", "App", "--nickname", "nick invalido"]
        ):
            with captured_output():
                with self.assertRaises(SystemExit) as cm:
                    cmd_new_project.main()
    ...
    def test_nickname_com_caractere_especial_retorna_erro(self):
        with patch(
            "sys.argv", ["new-project", "--name", "App", "--nickname", "nick@app"]
        ):
            with captured_output():
                with self.assertRaises(SystemExit) as cm:
                    cmd_new_project.main()
    ...
    def test_modo_interativo_com_nickname_customizado(self, _):
        with patch("sys.argv", ["new-project"]):
            with patch(
                "builtins.input", side_effect=["Meu Projeto", "meuprojeto", "/tmp"]
            ):
                with captured_output():
                    cmd_new_project.main()
        self.assertTrue(Project.objects.filter(nickname="meuprojeto").exists())

    def test_modo_interativo_eof_retorna_erro(self):
        with patch("sys.argv", ["new-project"]):
    ...
    def test_pycharm_e_aberto_apos_criacao(self, mock_popen):
        with patch(
            "sys.argv",
            ["new-project", "--name", "App", "--nickname", "app", "--dir", "/tmp"],
        ):
            with captured_output():
                cmd_new_project.main()
...
class TestStartSession(TestCase):
    ...
    def test_cria_sessao_para_projeto_valido(self, _):
        ...
        self.assertEqual(
            Session.objects.filter(
                project__nickname="myapp", ended_at__isnull=True
            ).count(),
            1,
        )
    ...
    def test_nova_sessao_criada_apos_auto_fechamento(self, _):
        ...
        self.assertEqual(
            Session.objects.filter(project=project, ended_at__isnull=True).count(), 1
        )
    ...
    def test_pycharm_aberto_quando_diretorio_existe(self, mock_popen):
        ...
        with patch("sys.argv", ["start-session", "myapp"]):
            with captured_output():
                cmd_start_session.main()
        pycharm_calls = [
            c for c in mock_popen.call_args_list if "watcher.py" not in str(c)
        ]
        self.assertEqual(len(pycharm_calls), 1)
    ...
    def test_pycharm_nao_aberto_quando_diretorio_inexistente(self, mock_popen):
        ...
        self.assertEqual(len(pycharm_calls), 0)
    ...
    def test_salva_notes_quando_fornecidas(self):
        ...
        with patch("sys.argv", ["end-session", "--notes", "implementei login"]):
            with captured_output():
                cmd_end_session.main()
        self.assertEqual(
            Session.objects.get(project__nickname="myapp").notes, "implementei login"
        )
...
class TestGenerateSessionSummary(TestCase):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            with captured_output() as out:
                result = generate_session_summary(
                    "/tmp", self._make_dt(60), self._make_dt(0)
                )
    ...
    def test_diretorio_inexistente_retorna_none(self):
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            with captured_output() as out:
                result = generate_session_summary(
                    "/caminho/que/nao/existe", self._make_dt(60), self._make_dt(0)
                )
        self.assertIsNone(result)
    ...
    def test_no_summarize_flag_pula_sem_perguntar(self):
        ...
        with patch("sys.argv", ["end-session", "--no-summarize"]):
            with patch(
                "builtins.input", side_effect=Exception("não deveria perguntar")
            ):
                with captured_output():
                    cmd_end_session.main()
...
def _load_watcher():
    loader = _SFL("watcher", _WATCHER_PATH)
    spec = _ilu.spec_from_file_location("watcher", _WATCHER_PATH, loader=loader)
...
class TestFileChangeHandler(TestCase):
    ...
    def setUp(self):
        ...
        self.project = make_project(nickname="myapp", path="/tmp")
        self.session = make_session(self.project)

    def _handler(self):
        return self.watcher.FileChangeHandler(self.session.pk)

    def _fake_event(self, path, is_directory=False):
        ev = MagicMock()
        ev.src_path = path
        ev.is_directory = is_directory
        return ev

    def test_regista_file_change_para_arquivo_python(self):
        self._handler().on_modified(self._fake_event("/tmp/myapp/main.py"))
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 1)

    def test_metadata_contem_arquivo_e_extensao(self):
        self._handler().on_modified(self._fake_event("/tmp/myapp/views.py"))
        ev = Event.objects.get(event_type="file_change")
        self.assertEqual(ev.metadata["file"], "/tmp/myapp/views.py")
        self.assertEqual(ev.metadata["extension"], ".py")

    def test_ignora_diretorios(self):
        self._handler().on_modified(
            self._fake_event("/tmp/myapp/subdir", is_directory=True)
        )
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 0)

    def test_ignora_extensao_pyc(self):
        self._handler().on_modified(
            self._fake_event("/tmp/myapp/__pycache__/views.cpython-312.pyc")
        )
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 0)

    def test_ignora_pasta_git(self):
        self._handler().on_modified(self._fake_event("/tmp/myapp/.git/COMMIT_EDITMSG"))
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 0)

    def test_ignora_pasta_venv(self):
        self._handler().on_modified(
            self._fake_event("/tmp/myapp/.venv/lib/python3.12/site.py")
        )
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 0)

    def test_on_created_tambem_registra(self):
        self._handler().on_created(self._fake_event("/tmp/myapp/novo.js"))
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 1)

    def test_nao_registra_quando_sessao_encerrada(self):
    ...
    def test_extensao_vazia_registrada_como_none(self):
        self._handler().on_modified(self._fake_event("/tmp/myapp/Makefile"))
        ev = Event.objects.get(event_type="file_change")
        self.assertEqual(ev.metadata["extension"], "(none)")

    def test_multiples_saves_registram_multiples_eventos(self):
        h = self._handler()
        h.on_modified(self._fake_event("/tmp/myapp/a.py"))
        h.on_modified(self._fake_event("/tmp/myapp/b.py"))
        h.on_modified(self._fake_event("/tmp/myapp/c.py"))
        self.assertEqual(Event.objects.filter(event_type="file_change").count(), 3)


class TestStartSessionWatcher(TestCase):

    @patch("subprocess.Popen")
    def test_watcher_iniciado_apos_criar_sessao(self, mock_popen):
        make_project(nickname="myapp", path="/tmp")
        with patch("sys.argv", ["start-session", "myapp"]):
            with captured_output():
                cmd_start_session.main()
        watcher_calls = [c for c in mock_popen.call_args_list if "watcher.py" in str(c)]
        self.assertEqual(len(watcher_calls), 1)

    @patch("subprocess.Popen")
    def test_watcher_recebe_session_id_correto(self, mock_popen):
        make_project(nickname="myapp", path="/tmp")
        with patch("sys.argv", ["start-session", "myapp"]):
            with captured_output():
                cmd_start_session.main()
        session = Session.objects.get(project__nickname="myapp")
        watcher_call = next(
            c for c in mock_popen.call_args_list if "watcher.py" in str(c)
        )
        self.assertIn(str(session.pk), watcher_call[0][0])

    @patch("subprocess.Popen")
    def test_watcher_usa_cwd_quando_path_inexistente(self, mock_popen):
        make_project(nickname="myapp", path="/caminho/que/nao/existe/xyz")
        with patch("sys.argv", ["start-session", "myapp"]):
            with captured_output():
                cmd_start_session.main()
        watcher_call = next(
        ...
        self.assertIn(os.getcwd(), watcher_call[0][0])


class TestEndSessionWatcher(TestCase):

    def test_mata_watcher_quando_pid_file_existe(self):
        project = make_project(nickname="myapp")
        make_session(project)
        with open("/tmp/devtracker_watcher.pid", "w") as f:
            f.write("99999")
        with patch("sys.argv", ["end-session", "--no-summarize"]):
    ...
    def test_nao_falha_quando_pid_file_ausente(self):
        project = make_project(nickname="myapp")
        make_session(project)
        if os.path.exists("/tmp/devtracker_watcher.pid"):
            os.remove("/tmp/devtracker_watcher.pid")
        with patch("sys.argv", ["end-session", "--no-summarize"]):
            with captured_output():
                cmd_end_session.main()
        self.assertFalse(Session.objects.filter(ended_at__isnull=True).exists())

    def test_nao_falha_quando_processo_inexistente(self):
        project = make_project(nickname="myapp")
        make_session(project)
        with open("/tmp/devtracker_watcher.pid", "w") as f:
            f.write("99999")
